...
def create_csv_output():
	...
	positive_feature_vectors = get_feature_vector (positive_images, True)
	negative_feature_vectors = get_feature_vector (negative_images, False)
...
def get_feature_vector(images, positive):
	feature_vector = []
	for src in images:
		...
		val_list = transfer_mat_values (resized, positive)
		feature_vector.append(val_list)

	return feature_vector

def transfer_mat_values (img, positive):
	val_list = []
	for x in range(0,8):
		for y in range(0,8):
			val = img[x][y]
